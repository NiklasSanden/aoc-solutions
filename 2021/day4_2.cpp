...
static void draw_and_remove_extra_winners(std::vector<Board>& boards, const int draw){
    ...
    if (boards.size() > 1) {
        auto [first, last]{
            std::ranges::remove_if(boards, [](const auto& b) { return b.has_won; })
        };
        boards.erase(first, last);
    }
}
...
std::string day4_2(std::istream& in)
{
    ...
    for (const auto draw : random_numbers) {
        draw_and_remove_extra_winners(boards, draw);
        if (boards.size() == 1ull && boards.front().has_won) {
            return std::to_string(draw * calculate_winning_board_sum(boards.front()));
        }
    }
